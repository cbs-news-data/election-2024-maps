...
county_data_unnest <- county_data %>% 
  spread_all() %>% 
  select(name, fips, precinctsOf, precinctsIn, pctPrecIn, pctExpVote, totalVote, timeStamp, candName, registration, totalVote, candVote, candPct) %>% #select only columns we need
